...
main(void)
{
    ...
    for (std::size_t i = 0; i < k_b; ++i) {
        std::size_t K = (i != k_b - 1 || K_ == 0) ? K_C : K_;
        for (std::size_t j = 0; j < n_b; ++j) {
            std::size_t N = (j != n_b - 1 || N_ == 0) ? N_C : N_;
            fmt::printf("B_{%zu,%zu} is a %zu x %zu matrix\n", i, j, K, N);

            // 5) Print the content of the matrix block B_{i,j}
            fmt::printf("B_{%zu,%zu} = \n", i, j);
            tools::printMatrix(K, N, &B[i * K_C * incRowB + j * N_C * incColB],
                               incRowB, incColB);

            // 6) Pack block B_{i,j} in buffer p
            ulmblas::pack_B(K, N, &B[i * K_C * incRowB + j * N_C * incColB],
                            incRowB, incColB, N_R, p);
